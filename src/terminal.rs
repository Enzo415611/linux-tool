...
use slint::{ComponentHandle, ToSharedString, Weak, format};
        ...
        while let Ok(n) = reader.read(&mut buffer) {
            ...
            slint::invoke_from_event_loop(move || {
                if let Some(ui) = ui_handle_clone.upgrade() {
                    ...
                    let current = logic.get_terminal_output().to_string();
                    logic.set_terminal_output(format!("{} {}", current, clear_output).to_shared_string());
                }
            }).unwrap();
        }
    });
